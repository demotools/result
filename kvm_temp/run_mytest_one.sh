...
XSBENCH_ARS=" -- -p 25000000 -g 920000 "
...
test_and_set_configs()
{
	...
	echo $NODE_MAX
        ...
        if [ $LAST_CHAR == "M" ]; then
                ...
                echo $numtest | sudo tee /proc/sys/kernel/pgtable_replication #> /dev/null
                if [ $? -ne 0 ]; then
...
launch_benchmark_config()
{
	...
	SECONDS=0
	if [ -f /tmp/alloctest-bench.done ]; then
		echo  "ERROR : benchmark already done!"
	fi
